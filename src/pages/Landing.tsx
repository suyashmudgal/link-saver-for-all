...
import {
  FolderOpen, Shield, Zap, ArrowRight, Star, Globe, Layers,
  Bell, Share2, BarChart3, BookOpen, Sparkles, ChevronRight, Check
} from "lucide-react";
...
const Landing = () => {
  const features = [
    { icon: FolderOpen, title: "Smart Folders", desc: "Nested folder system with drag-and-drop organization.", color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" },
    { icon: Shield, title: "Bank-Level Security", desc: "Row-level isolation ensures your data is truly private.", color: "bg-sky-500/10 text-sky-600 dark:text-sky-400" },
    { icon: Zap, title: "Instant Search", desc: "Find anything across all your content in milliseconds.", color: "bg-amber-500/10 text-amber-600 dark:text-amber-400" },
    { icon: Layers, title: "Multi-Format", desc: "Links, images, videos, and notes — all in one vault.", color: "bg-violet-500/10 text-violet-600 dark:text-violet-400" },
    { icon: Bell, title: "Smart Alerts", desc: "Get notified about dead links, digests, and reminders.", color: "bg-rose-500/10 text-rose-600 dark:text-rose-400" },
    { icon: BarChart3, title: "Usage Insights", desc: "Analytics dashboard to understand your saving habits.", color: "bg-blue-500/10 text-blue-600 dark:text-blue-400" },
    { icon: Share2, title: "Public Sharing", desc: "Share curated collections via public shareable pages.", color: "bg-teal-500/10 text-teal-600 dark:text-teal-400" },
    { icon: BookOpen, title: "Read Queue", desc: "Snooze links and resurface them when you're ready.", color: "bg-orange-500/10 text-orange-600 dark:text-orange-400" },
    { icon: Globe, title: "Access Anywhere", desc: "Your vault syncs across all devices, always available.", color: "bg-cyan-500/10 text-cyan-600 dark:text-cyan-400" },
  ];
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/70 backdrop-blur-2xl border-b border-border/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold tracking-tight" style={{ fontFamily: "'Outfit', sans-serif" }}>Info Trunk</span>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3">
            <ThemeToggle />
            <Link to="/auth"><Button variant="ghost" className="hidden sm:inline-flex rounded-xl">Login</Button></Link>
            <Link to="/auth?signup=true">
              <Button className="shadow-lg shadow-primary/25 rounded-xl" style={{ background: 'var(--gradient-primary)' }}>
      ...
      <section className="relative pt-32 pb-28 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="initial" animate="animate" variants={stagger} className="text-center">
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-8 text-sm font-medium text-foreground/80">
              <Sparkles className="w-3.5 h-3.5 text-primary" /> AI-Powered Knowledge Management
            </motion.div>

            <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl lg:text-8xl font-extrabold leading-[0.92] tracking-tight mb-6">
              Save Everything.
              <br />
              <span className="text-gradient animate-gradient">Find Anything.</span>
            </motion.h1>

            <motion.p variants={fadeUp} className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-12 leading-relaxed">
              The modern knowledge vault for links, notes, images & videos. Smart folders, instant search, and AI summaries — beautifully organized.
            </motion.p>

            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link to="/auth?signup=true">
                <Button size="lg" className="shadow-xl shadow-primary/30 px-8 text-base h-13 rounded-xl font-semibold" style={{ background: 'var(--gradient-primary)' }}>
            ...
            {/* Trust badges */}
            <motion.div variants={fadeUp} className="mt-10 flex items-center justify-center gap-6 text-xs text-muted-foreground">
              {["Free forever plan", "No credit card", "Instant setup"].map(t => (
        ...
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
              <div className="text-4xl md:text-5xl font-extrabold text-gradient">{s.value}</div>
      ...
      <section className="py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-muted-foreground max-w-xl mx-auto text-lg">Nine powerful features to organize your digital world.</p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className="group p-6 rounded-2xl bg-card/70 backdrop-blur-lg border border-border/40 hover:border-primary/30 transition-all duration-300 hover-lift gradient-border">
      ...
      <section className="py-28 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-extrabold mb-4">Three Steps to Organized</h2>
      ...
      <section className="py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-extrabold mb-4">Loved by Creators</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={t.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className="p-6 rounded-2xl bg-card/70 backdrop-blur-lg border border-border/40 hover-lift gradient-border">
        ...
        <div className="max-w-4xl mx-auto px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
            ...
            <div className="relative">
              <h2 className="text-3xl md:text-5xl font-extrabold text-primary-foreground mb-4">Ready to Organize Your Digital Life?</h2>
      ...
      <footer className="py-10 border-t border-border/30">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-sm" style={{ fontFamily: "'Outfit', sans-serif" }}>Info Trunk</span>
          </div>
